...
const OAuthButtons: React.FC<OAuthButtonsProps> = ({ onSuccess }) => {
  ...
  return (
    <div className="rent-oauth">
      <button
        type="button"
        className="rent-auth-method__btn rent-oauth__btn"
        onClick={() => handleClick('google')}
        disabled={!!loading}
      >
        {loading === 'google' ? (
          <span className="rent-auth-method__icon rent-oauth__spin" aria-hidden="true">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
              <path d="M21 12a9 9 0 1 1-6.219-8.56" />
            </svg>
          </span>
        ) : (
          <span className="rent-auth-method__icon" aria-hidden="true"><GoogleIcon /></span>
        )}
        {t('login.google')}
      </button>
      ...
      >
        {loading === 'apple' ? (
          ...
          <span className="rent-auth-method__icon" aria-hidden="true"><span className="rent-oauth__apple"><AppleIcon /></span></span>
        )}
        {t('login.apple')}
      </button>
