 ...
 * 持久化：Taro storageSync，冷启动同步读取，避免每次启动都回到默认城市。
 */
...
function readStorage(): string | null {
  try {
    const raw = Taro.getStorageSync(LOCATION_KEY) as string | null | undefined
    return raw || null
  } catch {
    return null
  }
...
function writeStorage(sel: LocationSelection): void {
  try {
    Taro.setStorageSync(LOCATION_KEY, JSON.stringify(sel))
  } catch {
    /* ignore */
  }
}
